...
public:
    ...
    extended(const result_type* data, state_type state)
        : baseclass(state)
    {
        datainit(data);
    }
    ...
    template <typename bc = baseclass>
    extended(state_type state, typename bc::stream_state stream_seed)
        : baseclass(state, stream_seed)
    {
        selfinit();
    }
